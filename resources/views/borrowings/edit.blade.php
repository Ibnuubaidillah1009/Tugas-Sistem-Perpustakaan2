...
@section('page-description', 'Perbarui informasi peminjaman buku')

@section('content')
<div class="max-w-2xl mx-auto">
    <div class="bg-white shadow rounded-lg">
        <div class="px-4 py-5 sm:p-6">
            <form method="POST" action="{{ route('perpustakawan.borrowings.update', $borrowing) }}" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" id="book_search" name="book_search" autocomplete="off"
                               value="{{ $borrowing->book->title }} - {{ $borrowing->book->author }}"
                               placeholder="Cari buku dengan judul, penulis, atau barcode"
                        ...
                        <input type="hidden" id="book_id" name="book_id" value="{{ old('book_id', $borrowing->book_id) }}" required />
                    ...
                    <div>
                        <label for="borrow_date" class="block text-sm font-medium text-gray-700">
                            Tanggal Pinjam <span class="text-red-500">*</span>
                        </label>
                        <input type="date" name="borrow_date" id="borrow_date" required
                               class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary-500 focus:border-primary-500 sm:text-sm @error('borrow_date') border-red-500 @enderror"
                    ...
                    <div>
                        ...
                        <select name="status" id="status" required
                                class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary-500 focus:border-primary-500 sm:text-sm @error('status') border-red-500 @enderror">
                    ...
                    <div>
                        ...
                        <textarea name="notes" id="notes" rows="3"
                                  ...
                                  placeholder="Catatan tambahan tentang peminjaman">{{ old('notes', $borrowing->notes) }}</textarea>
                        @error('notes')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
